...
export default function SubsApp({ session, theme, toggleTheme, recovery }) {
  ...
  let cats = Object.keys(map).map((k) => ({
    key: k, label: catOf(k).label, color: catOf(k).color, amount: map[k],
    pct: total ? (map[k] / total) * 100 : 0,
  }))
  cats = cats.filter((c) => c.amount > 0).sort((a, b) => b.amount - a.amount)
    ...
    <div className="frame">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--dim)', marginTop: 2 }}>{catOf(s.cat).label + ' · ' + fdate(s.next)}</div>
                      </div>
      ...
      {showNav && (
        <div style={{ position: 'sticky', bottom: 0, zIndex: 30, display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', alignItems: 'center', justifyItems: 'center', padding: '10px 8px calc(10px + env(safe-area-inset-bottom))', background: 'color-mix(in srgb, var(--bg) 85%, transparent)', backdropFilter: 'blur(20px)', borderTop: '1px solid var(--line)' }}>
          <NavBtn icon={<IconPanel />} label="Panel" on={screen === 'dashboard'} onClick={() => go('dashboard')} />
          <NavBtn icon={<IconStats />} label="Stats" on={screen === 'stats'} onClick={() => go('stats')} />
          ...
          <NavBtn icon={<IconCalendar />} label="Calendario" on={screen === 'calendar'} onClick={() => go('calendar')} />
          <NavBtn icon={<IconSettings />} label="Ajustes" on={screen === 'settings'} onClick={() => go('settings')} />
        </div>
      )}
...
function chargeDaysInMonth(sub, y, m) {
  if (!sub.next) return []
  // One-time payment: only appears in its own month, never repeats.
  if (sub.cycle === 'once') {
    const dd = new Date(sub.next + 'T12:00:00')
    return dd.getFullYear() === y && dd.getMonth() === m ? [dd.getDate()] : []
  }
...
const navSvg = { width: 25, height: 25, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 2, strokeLinecap: 'round', strokeLinejoin: 'round' }
...
function NavBtn({ icon, label, on, onClick }) {
  return (
    <button onClick={onClick} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, background: 'none', border: 'none', cursor: 'pointer', padding: '4px 4px', color: on ? 'var(--accent2)' : 'var(--faint)' }}>
      {icon}
      <div style={{ fontSize: 11, fontWeight: 600, whiteSpace: 'nowrap', letterSpacing: '-0.01em' }}>{label}</div>
    </button>
  )
}
      ...
      <div style={{ position: 'relative', background: `linear-gradient(165deg, ${sel.brand}, color-mix(in srgb, ${sel.brand} 55%, #000))` }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '56px 20px 24px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginTop: 14, color: '#fff' }}>
            ...
            <span style={{ fontSize: 14, color: 'rgba(255,255,255,0.7)' }}>{cyclePer(sel.cycle)}</span>
          </div>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      ...
      <div role="dialog" aria-modal="true" aria-label={mode === 'edit' ? 'Editar suscripción' : 'Nueva suscripción'} style={{ position: 'relative', width: '100%', maxWidth: 460, maxHeight: '92vh', overflowY: 'auto', background: 'var(--bg2)', borderRadius: '26px 26px 0 0', borderTop: '1px solid var(--line2)', boxShadow: '0 -10px 50px rgba(0,0,0,0.5)', animation: 'sheetUp .34s cubic-bezier(.16,1,.3,1)', padding: '8px 20px 28px' }}>
        ...
        <div style={{ display: 'flex', gap: 4, background: 'var(--panel)', border: '1px solid var(--line2)', borderRadius: 13, padding: 4 }}>
          {[['monthly', 'Mensual'], ['yearly', 'Anual'], ['quarterly', 'Trim.'], ['weekly', 'Sem.'], ['once', 'Único']].map(([k, l]) => (
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
          <div style={{ flex: 1 }}>
            <label style={{ ...label, marginBottom: 7 }}>{f.cycle === 'once' ? 'Fecha del pago' : 'Próximo cobro'}</label>
